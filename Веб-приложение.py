...
tasks = client.tasks.all().order_by('-priority', 'due_date')
...
goals = ClientGoal.objects.filter(client__assigned_to=request.user)
...
f'Среднее самочувствие: {avg_wellness.aggregate(Avg("avg"))["avg__avg"] or 0:.1f}',
...
f'({record["avg_wellness"]:.1f}/10) за последние 24 часа',
notification_type='wellness_alert',
